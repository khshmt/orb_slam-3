...
void LoopClosing::exportLoopClosureInfo(KeyFrame* pCurrentKF,
                                        KeyFrame* pMatchedKF, g2o::Sim3& g2oScw,
  ...
  if (std::filesystem::exists("loopClosureInfo") == false)
    std::filesystem::create_directory("loopClosureInfo");

  openOnce(kfFile, "loopClosureInfo/keyframes_loopClosing.csv",
           "#t_ns,x,y,z,qx,qy,qz,qw");

  openOnce(preintFile, "loopClosureInfo/imuPreint_loopClosing.csv",
  ...
  openOnce(covisFile, "loopClosureInfo/covisibility_loopClosing.csv",
  ...
  for (KeyFrame* pKFi : KFs) {
    ...
    if (pKFi->mpImuPreintegrated != nullptr && pKFi->mPrevKF != nullptr) {
      ...
      const Eigen::Vector3f dRv = Sophus::SO3<float>(dR).log();

      // Covariance diagonal
      const auto& C = pKFi->mpImuPreintegrated->C;

      preintFile << pKFi->mnId << ',' << std::setprecision(6)
                 << 1e9 * pKFi->mPrevKF->mTimeStamp << ','
                 << 1e9 * pKFi->mTimeStamp << ',' << std::setprecision(9)
    ...
    for (KeyFrame* pKFj : vpCovisKFs) {
      if (pKFj->isBad())
        continue;
      if (pKFj->mnId <= pKFi->mnId)
        continue;  // write each pair once

      const int nShared = pKFi->GetWeight(pKFj);

      covisFile << pKFi->mnId << ',' << pKFj->mnId << ',' << nShared << ','
                << std::setprecision(6) << 1e9 * pKFi->mTimeStamp << ','
                << 1e9 * pKFj->mTimeStamp << '\n';
    }
    covisFile.flush();
    ...
    for (KeyFrame* pKFj : loopEdges) {
      ...
      const long long edgeKey = static_cast<long long>(id_min) * 1000000LL +
                                static_cast<long long>(id_max);

      if (writtenLoopEdges.count(edgeKey))
      ...
      loopEdgeFile << id_min << ',' << id_max << ',' << std::setprecision(6)
                   << 1e9 * pKFi->mTimeStamp << ',' << 1e9 * pKFj->mTimeStamp
                   << ','
                   << std::setprecision(9)
